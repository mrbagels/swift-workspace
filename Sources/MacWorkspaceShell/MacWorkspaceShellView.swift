  ...
  >: View {
        ...
        HStack(spacing: 0) {
          if isSidebarVisible {
            MacWorkspaceSidebar(
              ...
              layout: configuration.layout,
              presentation: configuration.sidebarPresentation,
              searchPlaceholder: configuration.searchPlaceholder,
              theme: configuration.theme
            )
          ...
          contentRegion(width: metrics.contentWidth)
    ...
    private func contentRegion(width: CGFloat) -> some View {
    ...
    private var minimumWindowWidth: CGFloat {
      ...
      let sidebarFootprint = isSidebarVisible
        ? sidebarWidth + layout.dividerWidth
        : 0
  ...
  private struct MacWorkspaceSidebar<RouteID: Hashable & Sendable, FooterContent: View>: View {
    ...
    let presentation: MacWorkspaceSidebarPresentation
    ...
    init(
      ...
      presentation: MacWorkspaceSidebarPresentation,
    ...
    var body: some View {
      VStack(spacing: 0) {
        ...
        sidebarPanel
          .padding(.top, sidebarPanelTopInset)
          .padding(.leading, sidebarPanelHorizontalInset)
          .padding(.trailing, sidebarPanelHorizontalInset)
          .padding(.bottom, sidebarPanelBottomInset)
      }
    ...
    private var sidebarPanel: some View {
      VStack(spacing: 0) {
        searchButton
          .padding(.top, sidebarSearchTopInset)
        ...
        ScrollView(.vertical, showsIndicators: false) {
          ...
          .padding(.leading, sidebarContentLeadingInset)
          .padding(.trailing, sidebarContentTrailingInset)
          .padding(.vertical, 10)
      ...
      .background(
        RoundedRectangle(cornerRadius: sidebarPanelCornerRadius, style: .continuous)
          .fill(sidebarPanelBackground)
      )
      .overlay {
        if presentation == .floating {
          RoundedRectangle(cornerRadius: sidebarPanelCornerRadius, style: .continuous)
            .stroke(theme.divider.opacity(0.75), lineWidth: 1)
        }
      }
      .clipShape(RoundedRectangle(cornerRadius: sidebarPanelCornerRadius, style: .continuous))
      .accessibilityIdentifier("mac-workspace-sidebar-panel")
    }
    ...
    private var sidebarPanelBackground: Color {
      switch presentation {
      case .edgeToEdge:
        theme.canvas
      case .floating:
        theme.fill.opacity(0.62)
      }
    }

    private var sidebarPanelBottomInset: CGFloat {
      presentation == .floating ? 12 : 0
    }

    private var sidebarPanelCornerRadius: CGFloat {
      presentation == .floating ? 14 : 0
    }

    private var sidebarPanelHorizontalInset: CGFloat {
      presentation == .floating ? 12 : 0
    }

    private var sidebarPanelTopInset: CGFloat {
      switch presentation {
      case .edgeToEdge:
        max(0, layout.contentTopOffset - 8)
      case .floating:
        max(10, layout.contentTopOffset - 24)
      }
    }

    private var sidebarSearchTopInset: CGFloat {
      presentation == .floating ? 10 : 0
    }

    private var sidebarContentLeadingInset: CGFloat {
      switch presentation {
      case .edgeToEdge:
        max(24, layout.titlebarEdgePadding + 4)
      case .floating:
        12
      }
    }

    private var sidebarContentTrailingInset: CGFloat {
      switch presentation {
      case .edgeToEdge:
        max(12, layout.titlebarEdgePadding - 6)
      case .floating:
        12
      }
    }
        ...
        ForEach(visibleToolbarCommands) { command in
          Button {
            ...
            Label(command.title, systemImage: command.systemImage)
              ...
              .macWorkspaceHeaderControl(theme: theme, style: .custom)
          }
          .buttonStyle(.plain)
        ...
        if let primaryCommand = visiblePrimaryCommand {
          Button {
            ...
            Label(primaryCommand.title, systemImage: primaryCommand.systemImage)
              .labelStyle(.titleAndIcon)
              .macWorkspaceHeaderControl(theme: theme, isPrimary: true, style: .custom)
          }
          .buttonStyle(.plain)
  ...
  private struct MacWorkspaceTitlebarConfigurator: NSViewRepresentable {
    ...
    private final class ConfiguringView: NSView {
      ...
      init(layout: MacWorkspaceShellLayout) {
